from db_queries import get_all_employers_query, get_all_vacancies_query, get_avg_salary_query, get_high_salary_query
...
def user_interaction(db_manager):
    ...
    try:
        ...
        choice_two = input('Показать всех работодателей и количество вакансий от каждого? (1 - да), (2 - нет): ')
        if choice_two == '1':
        ...
        choice_three = input('Показать среднюю зарплату по всем вакансиям? (1 - да), (2 - нет): ')
        if choice_three == '1':
            print(f'\nСредняя зарплата по вакансиям равна {db_manager.get_avg_salary(get_avg_salary_query)} руб.\n')
        else:
            input('Для продолжения нажмите любую клавишу')

        choice_four = input('Показать вакансии с з/п выше средней? (1 - да), (2 - нет): ')
        if choice_four == '1':
        ...
        print(f'Ошибка: {e}')
        print('Возможно, произошла ошибка при выполнении запроса к базе данных.')
        print('Проверьте, что база данных заполнена корректно и повторите запросы.')
